...
def test_one_dataset(params, file_name, test_data, best_epoch):
    ...
    for p in model.parameters():
        if p.size(0) == model.n_o2id+1 and model.n_o2id > 0:
            print(p)
...
def get_auc(fold_num):
    ...
    # Datasets and Model
    parser.add_argument('--model', type=str, default='takt')
    ...
    if dataset in {"assist2009"}:
        ...
        params.n_tid = 0#8
        params.n_fid = 0#8
        params.n_sd = 0#14 #sequence_delay
        params.n_rd = 0#11 #repeat_delay
        params.n_xid = 0#816
        params.n_yid = 10#4
    ...
    if dataset in {"assist2017"}:
        ...
        params.n_tid = 0#4
        params.n_fid = 0#7
        params.n_sd = 0#18 #sequence_delay
        params.n_rd = 0#20 #repeat_delay
        params.n_xid = 0#16
        params.n_yid = 16#6
...
if __name__ == '__main__':
    ...
    for fold_num in range(5):
        test_q_num, auc, acc, rmse_a1, rmse_a2, loss, target_list, pred_list= get_auc(fold_num)
        total_q_num += test_q_num
        weight_auc += test_q_num * auc
        weight_acc += test_q_num * acc
        weight_a1_rmse += test_q_num * rmse_a1
        weight_a2_rmse += test_q_num * rmse_a2
    ...
    print('RMSE_A1:\t', weight_a1_rmse / total_q_num)
    print('RMSE_A2:\t', weight_a2_rmse / total_q_num)
