...
def train(net, params, optimizer, data, label):
    ...
    for idx in range(N):
        ...
        if model_type in transpose_data_model:
            ...
            input_qa1 = np.transpose(qa1_one_seq[:, :])  # Shape (bs, seqlen)
        ...
        target_a1 = (target_a1 - 1) / params.n_question
        target_2 = np.floor(target_a1)
        target_a2 = (target_a2 - 1) / params.n_question
        ...
        loss, pred, true_ct, pred_a1, true_a1_ct, pred_a2, true_a2_ct = net(input_q, input_qa, 
                        input_qa1, input_qa2, target, target_a1, target_a2, input_pid, input_tid,
                                input_fid, input_sd, input_rd, input_xid, input_yid)
    ...
    return loss, acc, auc, auc_a1, rmse_a2
...
def test(net, params, optimizer, data, label):
    ...
    for idx in range(N):
        ...
        with torch.no_grad():
            loss, pred, ct, pred_a1, ct_a1, pred_a2, ct_a2 = net(input_q, input_qa, input_qa1, input_qa2, target, target_a1,                                target_a2, input_pid, input_tid, input_fid, input_sd, input_rd, input_xid, input_yid)
    ...
    rmse_a2 = compute_rmse(all_a2_target, all_a2_pred)
    if label == 'Test':
        return loss, acc, auc, auc_a1, rmse_a2, all_target, all_pred
    else:
        return loss, acc, auc, auc_a1, rmse_a2
