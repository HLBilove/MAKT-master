...
class DATA(object):
    ...
    def load_data(self, path):
        ...
        for lineID, line in enumerate(file_data):
            ...
            if lineID % 6 == 5:
                feedbacks = line.split(self.separate_char)
                if len(feedbacks[len(feedbacks)-1]) == 0:
                    feedbacks = feedbacks[:-1]
                A = []
                A1 = []
                A2 = []
                for i in range(len(feedbacks)):
                    A.append(feedbacks[i].split(' ')[0])
                    A1.append(feedbacks[i].split(' ')[1])
                    A2.append(feedbacks[i].split(' ')[2])
                ...
                for k in range(n_split):
                    ...
                    for i in range(k * self.seqlen, endINdex):
                        if len(Q[i]) > 0:
                            ...
                            Yindex = int(S[i]) + round(float(A1[i])) * self.n_question
                            Zindex = int(S[i]) + round(float(A2[i])) * self.n_question
                    ...
                    qa1_data.append(a1_seq)
                    qa2_data.append(a2_seq)
                    p_data.append(p_seq)
                    t_data.append(t_seq)
                    f_data.append(f_seq)
        ...
        qa1_dataArray = np.zeros((len(qa1_data), self.seqlen))
        for j in range(len(qa1_data)):
            dat = qa1_data[j]
            qa1_dataArray[j, :len(dat)] = dat

        qa2_dataArray = np.zeros((len(qa2_data), self.seqlen))
        for j in range(len(qa2_data)):
            dat = qa2_data[j]
            qa2_dataArray[j, :len(dat)] = dat
        ...
        return q_dataArray, qa_dataArray, qa1_dataArray, qa2_dataArray, p_dataArray, t_dataArray, f_dataArray, \
    ...
    def load_test_data(self, path):
        ...
        for lineID, line in enumerate(file_data):
            ...
            if lineID % 6 == 2:
                ...
                for i in range(len(Performance)):
                    T.append(Performance[i].split(' ')[0])
                    F.append(Performance[i].split(' ')[2])
            if lineID % 6 == 3:
                Forget = line.split(self.separate_char)
                if len(Forget[len(Forget)-1]) == 0:
                    Forget = Forget[:-1]
